...
export async function GET(
  ...
  try {
    ...
    // Validate parameters
    if (!plan_code || !age) {
      return NextResponse.json({ error: 'Missing plan_code or age parameter' }, { status: 400 })
    }
    ...
    if (isNaN(planCodeNumber) || planCodeNumber < 0 || planCodeNumber > 18) {
      return NextResponse.json(
        { error: 'Plan code must be a valid number between 0 and 18' },
        { status: 400 },
      )
    }
