...
const lockEmailExceptSuperAdmin = async ({ data, originalDoc, req, operation }: any) => {
  if (operation !== 'update') {
    return data
  }
  ...
  if (hasRole(currentUser, [ROLES.SUPER_ADMIN])) {
    return data
  }

  // Payload auth email may come with the form even when user is editing other fields.
  // So instead of blocking the save, always restore the original email.
  ...
  // Extra safety: if original email is missing, remove submitted email.
...
export const Users: CollectionConfig = {
  ...
  hooks: {
    ...
    beforeChange: [lockEmailExceptSuperAdmin],
