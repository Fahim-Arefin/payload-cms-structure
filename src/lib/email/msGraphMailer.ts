...
type GraphAttachment = {
  ...
  // Normal file attachments will continue to work without these.
  isInline?: boolean
  contentId?: string
}
...
export async function sendEmailViaMsGraph(input: SendViaGraphInput) {
  ...
  if (!sender) {
    throw new Error('MS_SENDER_EMAIL is missing')
  }
      ...
      ...(input.attachments?.length
        ? {
            attachments: input.attachments.map((attachment) => ({
              ...
              // Required only for inline images:
              // <img src="cid:sagar-logo" />
