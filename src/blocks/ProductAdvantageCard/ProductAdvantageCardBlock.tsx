...
import ProductAdvantageCardCarousel from './components/ProductAdvantageCardCarousel'
...
function ProductAdvantageCardBlock({ block }: Props) {
  ...
  const isCarousel = block?.layout === 'carousel' // <-- adjust to your field name

  return (
    <WithHashScroller id={block?.sectionId} bgColor={block?.backgroundColor}>
      <div className={`${isCarousel ? 'container-padding-y' : 'container-padding'} `}>
        {/* section intro */}
        <IntroSection block={block} className={`${isCarousel && 'container-padding-x'}`} />
        {/* grid */}
        {!isCarousel && (
          <div
        ...
        {isCarousel && (
          <div className="mt-4 lg:mt-6 xl:mt-8 2xl:mt-10 ">
            <ProductAdvantageCardCarousel block={block} />
          </div>
        )}
      </div>
    </WithHashScroller>
