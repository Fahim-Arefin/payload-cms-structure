...
type ProjectApproach = NonNullable<ProjectApproachBlockType['projectApproach']>

type Props = {
  data: ProjectApproach['approachItems'][number]
  index: number
}
...
function ProjectApproachCard({ data, index }: Props) {
  ...
  return (
    <div
      className={`
        project-approach-card-inner
        ${index % 2 === 0 ? 'bg-secondary-1' : 'bg-primary-1'}
      ...
      `}
    >
      <div className="relative z-10 flex min-h-[26px] lg:min-h-[30px] xl:min-h-[36px] items-center">
      ...
      {hasDesc && (
        <div
          className="
            project-approach-card-description
            relative z-10 mt-3 lg:mt-[14px] xl:mt-[18px]
            font-grift text-justify global-p4 text-white-1
