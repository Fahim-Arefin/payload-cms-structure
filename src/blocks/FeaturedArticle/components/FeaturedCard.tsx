...
function FeaturedCard({ block, data, detailsPage = false }: Props) {
  ...
  return (
    ...
    >
      ...
      <Link
        href={articleHref}
        aria-label={data?.title ? `Read article: ${data.title}` : 'Read featured article'}
        className="
            ...
            {selectedTags.map((tag, index) => (
              <span
                ...
                className="
                  ...
                  text-secondary-1
                  transition-all duration-300 ease-out
                  group-hover/featured-card:border-primary-1
                  group-hover/featured-card:bg-primary-1
                  group-hover/featured-card:text-white-1
                "
              >
                {tag.label}
              </span>
