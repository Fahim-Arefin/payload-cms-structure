  ...
  fields: [
    ...
    /* ---------------- Audience Cards (mobile only) ---------------- */
    {
      ...
      fields: [
        ...
        // if you later want text, you can re-enable the text / textBN row here
      ],
    },

    /* ---------------- Expectations – LEFT (plain array) ---------------- */
    {
      name: 'expectationsLeft',
      type: 'array',
      required: true,
      minRows: 4,
      maxRows: 12,
      label: 'Expectations – Left Bullets',
      labels: { singular: 'Bullet', plural: 'Bullets' },
      admin: {
        description: 'Bullets with icon + EN/BN short text.',
      },
      fields: [
        ...generateArrayImageFields({
          fieldName: 'icon',
    ...
    /* ---------------- Expectations – RIGHT (plain array min=1,max=1) ---------------- */
    {
      name: 'expectationsRight',
      type: 'array',
      required: true,
      minRows: 1,
      maxRows: 1,
      label: 'Expectations – Right Card (Avatar + Quote)',
      labels: { singular: 'Right Card', plural: 'Right Card' },
      admin: {
        description: 'Single agent testimonial: avatar + EN/BN name + EN/BN quote.',
      },
      fields: [
        ...generateArrayImageFields({
          fieldName: 'avatar',
          label: 'Avatar',
          description: 'Agent photo (1:1 portrait preferred and also prefer transparent img).',
          aspectRatio: 1,
          quality: 1,
          maxKB: 300,
          ownerCollection: AGENT_ONBOARDING_PAGE_AGENT_ONBOARDING_OPPORTUNITY_SLUG_AND_TAG as any,
