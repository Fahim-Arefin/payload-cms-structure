...
export default function OnboardingOpportunity({ data }: Props) {
  ...
  return (
    <div
      className="container-padding"
      style={{
        backgroundColor: data?.backgroundColor || '#FCF4EB',
      }}
    >
      {/* Desktop heading */}
      <div className="global-h1 hidden md:block font-semibold text-[#434342] uppercase lg:block">
      ...
      <div className="flex flex-col">
        {/* Opportunity Section – MOBILE only */}
        <section
          className="md:hidden"
          style={{
            backgroundColor: data?.backgroundColor || '#FCF4EB',
                  ...
                  <div className="relative w-full aspect-[1/1] rounded-md overflow-hidden">
                    {typeof item?.image === 'object' && item?.image?.url && (
                      <Image
                        ...
                        sizes="50vw"
                        placeholder="blur"
                        blurDataURL={(item as any)?.imageBlurDataURL || ''}
                      />
                    )}
                    <div className="absolute inset-0 bg-black/30 flex items-center justify-center px-4 text-center">
                      {/* If later you add text/textBN for cards, render them here */}
                    </div>
                  </div>
                </CarouselItem>
              ))}
        ...
        {/* Expectations Section: LEFT + RIGHT columns */}
        <div
          className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-10 
        md:mt-8 lg:mt-12 xl:mt-16 2xl:mt-20"
        >
          ...
          <div>
            ...
            py-4"
            >
              {data?.expectationsLeft?.map((item, idx) => (
                <div key={idx} className="flex items-center gap-6">
                  <div className="mt-1 relative w-[40px] lg:w-[48px] aspect-[1/1]">
                    {typeof item?.icon === 'object' && item?.icon?.url && (
                      <Image
                        ...
                        placeholder="blur"
                        blurDataURL={(item as any)?.iconBlurDataURL || ''}
                      />
                    )}
                  </div>
          ...
          {/* Right testimonial */}
          {data?.expectationsRight?.map((item, index) => (
            <div
              key={index}
              className="bg-[#FFFFFF8C] rounded-md p-6 shadow-sm text-center flex flex-col items-center justify-center gap-2 lg:gap-6"
            >
              <div className="relative w-[100px] h-[100px] lg:w-[208px] lg:h-[208px] mb-4">
                {typeof item?.avatar === 'object' && item?.avatar?.url && (
                  <Image
                    fill
                    src={item.avatar.url}
                    alt={item?.name || 'Agent avatar'}
                    className="object-cover object-center"
                    sizes="400px"
                    placeholder="blur"
                    blurDataURL={(item as any)?.avatarBlurDataURL || ''}
                    quality={85}
                  />
                )}
              </div>
