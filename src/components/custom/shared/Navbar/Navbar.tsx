...
// import type { HeaderData } from '../ServerTopHeader'
// import type { NavbarData, NavItem } from './ServerNavbar'

// type Props = { data: NavbarData; header: HeaderData; blur: string }

// export default function Navbar({ data, header, blur }: Props) {
...
//     if (!href || href === '#') return false // ✨ guard empty/placeholder links
...
//     return (
//       <ul
//         className={`${positionClass} ${beforeBridge}
...
//                 {hasChildren && (
//                   <RiArrowDownSLine
//                     className={`ml-2 w-[16px] h-[16px] transition-transform duration-200 ${
...
//           <div className="relative w-[66px] xl:w-[69px] 2xl:w-[75px] aspect-[1.48/1]">
...
//         {/* Desktop Menu */}
//         <ul className="hidden lg:flex lg:space-x-2 xl:space-x-3 2xl:space-x-4 lg:text-[14px] xl:text-[16px] 2xl:text-[18px] text-[#1E1E1E] items-center lg:leading-6 xl:leading-8">
...
//                 >
//                   <div
//                     className={`flex items-center space-x-1 ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
//                   >
//                     <div>
//                       <LocalizedText en={item.label} bn={item.labelBN} />
//                     </div>
...
//           <div className="relative w-[66px] xl:w-[69px] 2xl:w-[75px] aspect-[1.48/1]">
//             <Link href="/" aria-label="Home">
//               <Image src={logoUrl} alt="Company logo" fill className="object-contain" priority />
//             </Link>
//           </div>
...
export default function Navbar({ data, header, blur }: Props) {
  ...
  const [showNavbar, setShowNavbar] = useState(true)
  const [mobileDropdowns, setMobileDropdowns] = useState<Record<string, boolean>>({})
  // 👇 NEW: track current hash so we can distinguish /support#form vs /support#faq
  ...
  const isActive = (href?: string) => {
    ...
    const [baseHref, hashPart] = href.split('#') // "/support", "form" (maybe undefined)
    const pathMatch = pathname === baseHref || pathname.startsWith(baseHref + '/')

    // No hash in the link → normal page-level active (e.g. top-level "Support")
    if (!hashPart) {
      return pathMatch
    }

    // Link has a section id → require hash to match too
    const normalizedCurrentHash = currentHash.startsWith('#') ? currentHash.slice(1) : currentHash

    return pathMatch && normalizedCurrentHash === hashPart
  }
