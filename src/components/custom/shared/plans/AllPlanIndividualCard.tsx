...
function AllPlanIndividualCard({ data, blur }: Props) {
  return (
    <Link
      href={data?.link}
      className="group relative overflow-hidden w-[70%] mx-auto md:w-full 
    h-[250px] md:h-[250px] xl:h-[330px] 2xl:h-[400px] 
    ...
    >
      ...
      >
        <div className="h-[50%] flex items-start">
          <h3 className="text-[12px] lg:text-[12px] xl:text-[18px] 2xl:text-[20px] font-medium uppercase">
            {data?.title}
            <br></br>
            {data?.biggerTitle && (
              <p className="text-[14px] lg:text-[14px] xl:text-[20px] 2xl:text-[28px] font-medium uppercase">
                {data?.biggerTitle}
              </p>
            )}
          </h3>
        ...
        h-[60%] lg:w-[80%] flex flex-col justify-between items-center md:items-start`}
        >
          <p className="global-p2 text-center md:text-start">
            {data?.description &&
...
export default AllPlanIndividualCard
