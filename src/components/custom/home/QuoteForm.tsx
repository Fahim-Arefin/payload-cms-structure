...
import LocalizedString from '../shared/LocalizedString'
import LocalizedText from '../shared/LocalizedText'
import { PremCalculatorPageBlockType, PremiumCalculatorBlockType } from '@/types/payloadCustomTypes'
import LocalizedRichText from '../shared/LocalizedRichText'
...
interface QuoteFormProps {
  ...
  setFormData: React.Dispatch<React.SetStateAction<FormData>>
  onApiResponse?: (response: ApiResponse, paymentMode: string) => void
  payloadData: PremiumCalculatorBlockType | PremCalculatorPageBlockType
  // ✅ new
  onMetaChange?: (patch: Partial<QuoteMeta>) => void
  onMetaReset?: () => void
  setIsPlanChanged: React.Dispatch<React.SetStateAction<boolean>>
...
// interface PageQuoteFormProps {
//   homepage: false
//   formData: FormData
//   setFormData: React.Dispatch<React.SetStateAction<FormData>>
//   onApiResponse?: (response: ApiResponse, paymentMode: string) => void
//   payloadData: PremCalculatorPageBlockType
//   // ✅ new
//   onMetaChange?: (patch: Partial<QuoteMeta>) => void
//   onMetaReset?: () => void
//   setIsPlanChanged: React.Dispatch<React.SetStateAction<boolean>>
...
// type QuoteFormProps = HomeQuoteFormProps | PageQuoteFormProps
...
function QuoteForm({
  ...
  return (
    <form
      onSubmit={handleSubmit}
      action=""
      className={`${homepage ? 'bg-[#FFFFFFCC] border-2 border-[#9C8639]' : 'bg-[#9C863940]'} rounded-lg xl:rounded-xl 2xl:rounded-2xl
                 grid grid-cols-2 gap-x-6 lg:gap-x-4 xl:gap-x-6 gap-y-7 md:gap-y-7 lg:gap-y-[31px] xl:gap-y-[32px] 2xl:gap-y-[38px]
                 p-4 py-6 md:p-6 lg:p-5 xl:p-8 z-10`}
    >
      {/* DOB and Age Fields */}
      <div className="col-span-2 md:col-span-1">
        <div className="flex gap-2">
          ...
          <div className="flex-1">
            <Button
              variant={`${homepage ? 'outline' : 'default'}`}
              disabled
              className={`${homepage ? '' : '!bg-white'} w-full justify-start text-left font-normal !text-[12px] md:!text-[14px] 2xl:!text-[16px]
               placeholder:!text-[12px] md:placeholder:!text-[14px] 2xl:placeholder:!text-[16px]
               rounded-sm lg:rounded-[9px] xl:rounded-[10px]
               shadow-[0px_0px_5px_0px_#00000040] px-3 py-5 xl:px-4 xl:py-6
      ...
      >
        ...
        >
          <SelectTrigger
            aria-label="Select Tenure"
            className={`${homepage ? '' : 'bg-white'} !text-[12px] md:!text-[14px] 2xl:!text-[16px] 
                        placeholder:!text-[12px] md:placeholder:!text-[14px] 2xl:placeholder:!text-[16px]
                        rounded-sm lg:rounded-[9px] xl:rounded-[10px] shadow-[0px_0px_5px_0px_#00000040] px-5 py-5 xl:px-6 xl:py-6
                        ${
                          isLoadingTenures ||
      ...
      {/* SUBMIT */}
      <div className={`col-span-2 ${homepage ? '' : 'mx-auto'}`}>
        <GlobalButton
          type="submit"
          variant="secondary"
          disabled={isLoading || !agreeTerms}
          aria-disabled={isLoading || !agreeTerms}
          size={homepage ? 'small' : 'large'}
          // className={[
          //   homepage ? '' : 'bg-[#978900]',
          //   'px-5 py-5 xl:px-6 xl:py-6 w-full md:w-full lg:w-full xl:w-full 2xl:w-full',
          //   'rounded-sm lg:rounded-[9px] xl:rounded-[10px]',
          ...
          className={[
            homepage
              ? 'px-5 py-5 xl:px-6 xl:py-6 w-full md:w-full lg:w-full xl:w-full 2xl:w-full rounded-sm lg:rounded-[9px] xl:rounded-[10px]'
              : 'bg-[#978900] disabled:bg-gray-400 w-fit md:global-h4 text-white rounded-sm lg:rounded-[9px] xl:rounded-[10px] px-6 py-4 xl:px-8 xl:py-8',
            !agreeTerms || isLoading ? 'opacity-60 cursor-not-allowed' : '',
          ].join(' ')}
        >
          {' '}
          ...
          {homepage ? (
            <LocalizedString
              en={isLoading ? 'Calculating...' : 'Calculate Your Premium'}
              bn={isLoading ? 'হিসাব করা হচ্ছে...' : 'আপনার প্রিমিয়াম ক্যালকুলেট করুন'}
            />
          ) : (
            <LocalizedString
              en={isLoading ? 'Calculating...' : 'Calculate Now'}
              bn={isLoading ? 'হিসাব করা হচ্ছে...' : 'এখনই ক্যালকুলেট করুন'}
            />
          )}
        </GlobalButton>
      </div>
      ...
      {!homepage && (
        <div className="col-span-2 text-[12px] md:text-[14px] text-[#00000099] mt-4 md:mt-1 md:w-[90%] font-light md:capitalize mx-auto">
          {L(
