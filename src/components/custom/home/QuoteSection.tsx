...
import PlanDetailsSectionByCode from './PlanDetailsSectionByCode'
...
// type PageProps = {
//   homepage: false
//   data: PremCalculatorPageBlockType
//   footerData: GlobalFooter
// }

// type Props = HomeProps | PageProps

type QuoteSectionHomeProps = {
  homepage: true
  data: PremiumCalculatorBlockType
  footerData: GlobalFooter
}

type QuoteSectionPageProps = {
  homepage: false
  data: PremCalculatorPageBlockType
  footerData: GlobalFooter
}

type QuoteSectionProps = QuoteSectionHomeProps | QuoteSectionPageProps
...
function QuoteSection(props: QuoteSectionProps) {
  const { footerData, data, homepage } = props
  ...
  return (
    ...
    <div className="relative font-avenir container-width container-padding-y px-2 lg:px-0">
      ...
      {!homepage && (
        <div className="flex flex-col items-start justify-start mb-6 lg:mb-10 xl:mb-20">
      ...
      >
        ...
        >
          ...
          {!homepage && !apiResponse && (
            <div>
              {data?.cards?.map((card, idx) => {
                ...
                const isFirst = idx === 0
                const isLast =
                  data?.cards?.length === undefined ? false : idx === data?.cards?.length - 1
          ...
          {!homepage && apiResponse && <PlanDetailsSectionByCode planCode={formData?.PlanCode} />}
                      ...
                      >
                        <AnimatedCounter
                          value={Math.ceil(
                            confirmedPaymentMode === 'Quarterly'
                              ? getTotalPremiumWithCoverage('Quarterly')
                              : getTotalPremium(apiResponse, 'Quarterly')?.lifePremium.quarterly ||
                                  0,
                          )}
                          prefix="৳"
                          showAnimation={confirmedPaymentMode === 'Quarterly'}
                  ...
                  (formData?.PlanCode === 6 ||
                    ...
                    <div className="flex justify-center items-center mb-2">
                      ...
                      >
                        <LocalizedString
                          en="Get A Quote Now"
                          bn="আপনার প্রিমিয়াম ক্যালকুলেট করুন"
                        />
                      </GlobalButton>
