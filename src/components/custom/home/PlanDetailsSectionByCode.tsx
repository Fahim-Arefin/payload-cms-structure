...
type PlanDetailsSectionByCodeProps = {
  planCode: number
}

const PlanDetailsSectionByCode: FC<PlanDetailsSectionByCodeProps> = ({ planCode }) => {
  //   1 for child educatioon
  //   2 for endowment
  //   3 for deposite
  const keyFeatures = {
    1: [
    ...
    3: ['Flexible Premium Payments', 'Healthcare Partner Discounts'],
  }
  ...
  //   endowment = 5 single endowment = 14
  //   child education 8-13
  if ((planCode >= 8 && planCode <= 13) || planCode === 5 || planCode === 14) {
    const fakeCode = planCode === 5 || planCode === 14 ? 2 : 1

    const features = keyFeatures[fakeCode]
    return (
  ...
  if (planCode >= 15 && planCode <= 17) {
    const features = keyFeatures[3]
    return (
      <div className="w-full flex flex-col items-start gap-6 mb-6">
        <h3 className="global-h3 font-semibold uppercase text-left">
          {planCode === 1 ? 'Shanta Child Education Plan' : 'Shanta Endowment Plan'}
...
export default PlanDetailsSectionByCode
