...
// function CalculateForm({ onApiResponse, formData, setFormData, consentEn, consentBn }: Props) {
//   // ===== Localization helper =====
...
//     { labelEn: string; labelBn: string; value: number }[]
//   >([])
...
//         const children = normalized.filter((p) => p.key.toLowerCase().includes('child education'))
...
//               value={
//                 formData.Age
//                   ? `${L('Age', 'বয়স')}: ${lang === 'en' ? formData.Age : bnNum(formData.Age)}`
//                   : ''
//               }
//               readOnly
...
//               >
//                 {(() => {
//                   const pickedRegular = availablePlans.find(
...
//                   )
//                   const label =
//                     pickedRegular?.[lang === 'en' ? 'labelEn' : 'labelBn'] ??
//                     pickedChild?.[lang === 'en' ? 'labelEn' : 'labelBn'] ??
//                     ''
...
//                 {L('Watch', 'ভিডিও দেখুন')}{' '}
//                 {lang === 'en' ? selectedPlan.labelEn : selectedPlan.labelBn} {L('Video', '')}
//               </p>
//             </DialogTrigger>
//             <DialogContent
...
//                 : bnNum(suggestedAmount.toLocaleString())}
//             </span>{' '}
//             {L('BDT', 'টাকা')}
//           </p>
//         )}
...
import { Button } from '@/components/ui/button'
import type { ApiResponse } from '@/utils/premiumCalculator'
import { Dialog, DialogContent, DialogTitle, DialogTrigger } from '@/components/ui/dialog'
import { VisuallyHidden } from '@radix-ui/react-visually-hidden'
import { Checkbox } from '@/components/ui/checkbox'
import Link from 'next/link'
...
type Props = {
  onApiResponse?: (response: ApiResponse, paymentMode: string, planName?: string) => void
  onResetResult?: () => void
  onMetaChange?: (patch: QuoteMetaPatch) => void
  formData: FormData
  setFormData: React.Dispatch<React.SetStateAction<FormData>>
  consentEn?: any
  consentBn?: any
...
/** ✅ Canonical EN keys -> labels (EN/BN) (updated with new plans) */
const PLAN_LABELS = {
...
/** ✅ API → Canonical EN mapping (updated) */
const API_TO_CANONICAL: Record<string, CanonicalKey> = {
  'Shanta Endowment': 'Shanta Endowment Plan',
  'Shanta Three Payment Plan': 'Shanta 3 Stage Plan',
  'Shanta Four Payment Plan': 'Shanta 4 Stage Plan',
  ...
  // ✅ NEW (Block-1) – map the exact API names you’re using
...
/** Canonical EN -> video URL (keep your exact mapping rules) */
const VIDEO_LINKS: Record<CanonicalKey, string> = {
  'Shanta Child Education Plan (1%)': 'https://www.youtube.com/embed/Fj_BE9D64W4',
  'Shanta Child Education Plan (2%)': 'https://www.youtube.com/embed/Fj_BE9D64W4',
  'Shanta Child Education Plan (3%)': 'https://www.youtube.com/embed/Fj_BE9D64W4',
  ...
  // new plans: keep same/empty if you don’t want video
  'Shanta Single Premium Endowment': '',
  'Shanta Depositor’s Protection Plan - Signature': '',
  'Shanta Depositor’s Protection Plan - Elite': '',
  'Shanta Depositor’s Protection Plan - Prime': '',
...
function CalculateForm({
  ...
  const [availablePlans, setAvailablePlans] = useState<AvailablePlan[]>([])
  const [childEducationVariants, setChildEducationVariants] = useState<AvailablePlan[]>([])
  const [depositorVariants, setDepositorVariants] = useState<AvailablePlan[]>([])
  const [isLoadingPlans, setIsLoadingPlans] = useState(false)
  const [planError, setPlanError] = useState<string | null>(null)
  const [planMenuOpen, setPlanMenuOpen] = useState(false)
  ...
  // ✅ mobile plan selector behavior (Block-1)
  const [isMobile, setIsMobile] = useState(false)
  const [mobileChildOpen, setMobileChildOpen] = useState(false)
  const [mobileDepositorOpen, setMobileDepositorOpen] = useState(false)
  ...
  const suggestedAmount = useMemo(() => {
    ...
    return 100000
  }, [formData.Term, formData.annualIncome])

  // ✅ Fetch plans (updated with depositor + new plans)
  const fetchPlans = async (age: number) => {
    if (!age || age < 18 || age > 65) {
      setAvailablePlans([])
      setChildEducationVariants([])
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const children = normalized.filter((p) => p.key.toLowerCase().includes('child education'))
        const depositor = normalized.filter((p) => p.key.toLowerCase().includes('depositor'))
        const others = normalized.filter(
          (p) =>
            !p.key.toLowerCase().includes('child education') &&
            !p.key.toLowerCase().includes('depositor'),
        )

        setChildEducationVariants(children)
        setDepositorVariants(depositor)
        setAvailablePlans(others)
      } else {
        setAvailablePlans([])
        setChildEducationVariants([])
  ...
  const selectPlan = (plan: AvailablePlan) => {
    // ✅ reset result when plan changes (Block-1 behavior)
    onResetResult?.()
    setError(null)
    ...
    setAvailablePaymentModes([])
    setCurrentPaymentMode('')
    setFieldErrors((prev) => ({ ...prev, PlanCode: false, Term: false, PaymentMode: false }))

    // ✅ meta patch (Block-1 behavior)
    onMetaChange?.({
      lang,
      plan: {
        id: plan.plan_code,
        displayName: lang === 'en' ? plan.labelEn : plan.labelBn,
      },
    })
  }

  // Age change => fetch plans & reset plan selections
  useEffect(() => {
    if (formData.Age) {
      fetchPlans(formData.Age)

      // ✅ reset result when age causes plan reset
      onResetResult?.()
      setError(null)
      ...
      onMetaChange?.({
        lang,
        plan: { id: 0, displayName: '' },
        term: { id: 0, displayName: '' },
        payment: { id: 0, displayName: '' },
      })
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData.Age])
  ...
  const calculatePremium = async () => {
    ...
    try {
      ...
      const data: ApiResponse[] = await response.json()
      if (data && data.length > 0) {
        // pass canonical plan key to parent (keeps your existing behavior)
        onApiResponse?.(data[0], currentPaymentMode, selectedPlan?.key)
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred')
    ...
    >
      ...
      >
        ...
        >
            ...
            >
              <span className="truncate">
                {(() => {
                  const picked = [
                    ...availablePlans,
                    ...childEducationVariants,
                    ...depositorVariants,
                  ].find((p) => p.plan_code === formData.PlanCode)
                  const label = picked ? (lang === 'en' ? picked.labelEn : picked.labelBn) : ''
          ...
          >
            <div className="py-2">
              ...
              {depositorVariants.length > 0 && (
                <>
                  ...
                  {isMobile ? (
                    <>
                      <DropdownMenuItem
                        onClick={() => setMobileDepositorOpen((p) => !p)}
                        className="px-3 py-2 cursor-pointer flex items-center justify-between"
                      >
                        <span>
                          {L('Shanta Depositor’s Protection Plan', 'ডিপোজিটরস প্রোটেকশন প্ল্যান')}
                        </span>
                      ...
                      {mobileDepositorOpen &&
                        depositorVariants.map((variant) => {
                          ...
                          return (
                            <DropdownMenuItem
                              key={variant.plan_code}
                              onClick={() => selectPlan(variant)}
                              className={`pl-7 pr-3 py-2 cursor-pointer flex items-center gap-2 ${selected ? 'bg-accent text-accent-foreground' : ''}`}
                            >
                              <Check
                                className={`h-4 w-4 ${selected ? 'opacity-100' : 'opacity-0'}`}
                              />
                    ...
                    <DropdownMenuSub>
                      ...
                      <DropdownMenuSubContent className="z-[1100] min-w-[320px] rounded-md border bg-popover text-popover-foreground shadow-md p-0 overflow-hidden">
                        {depositorVariants.map((variant) => {
                          const selected = formData.PlanCode === variant.plan_code
                          const label = lang === 'en' ? variant.labelEn : variant.labelBn
                          return (
              ...
              {/* ✅ Child Education group (Block-1 mobile behavior) */}
              {childEducationVariants.length > 0 && (
                <>
                  <DropdownMenuSeparator />
                  ...
                  {isMobile ? (
                    <>
                      <DropdownMenuItem
                        onClick={() => setMobileChildOpen((p) => !p)}
                        className="px-3 py-2 cursor-pointer flex items-center justify-between"
                      >
                        <span>
                          {L('Shanta Child Education Plan', 'শান্তা চাইল্ড এডুকেশন প্ল্যান')}
                        </span>
                        <span className="text-xs opacity-70">{mobileChildOpen ? '—' : '+'}</span>
                      </DropdownMenuItem>
        ...
        {selectedPlan && selectedPlan.videoLink && (
          <Dialog>
            ...
            <DialogContent className="max-w-5xl w-full aspect-video p-0 bg-black [&>button.absolute]:top-3 [&>button.absolute]:right-3 [&>button.absolute]:bg-black/50 [&>button.absolute]:text-white [&>button.absolute]:hover:bg-black/80">
              <VisuallyHidden>
                <DialogTitle>Plan Video</DialogTitle>
              </VisuallyHidden>
              <iframe
        ...
        {/* ✅ show plan fetch error on UI (Block-1) */}
        {planError && <p className="text-red-600 text-xs mt-1">{planError}</p>}
        {getFieldErrorMessage('PlanCode') && (
          <p className="text-red-500 text-xs mt-1">{getFieldErrorMessage('PlanCode')}</p>
        )}
      </div>
      ...
      >
        <Select
          ...
          onValueChange={(v) => {
            const val = parseInt(v, 10)
            if (!isNaN(val)) {
              handleInputChange('Term', val)
              onMetaChange?.({
                lang,
                term: {
                  id: val,
                  displayName: lang === 'en' ? `${val} years` : `${bnNum(val)} বছর`,
                },
              })
            }
          }}
      ...
      <div className="col-span-2 md:col-span-1">
        <Select
          ...
          onValueChange={(v) => {
            const id = parseInt(v, 10)
            handleInputChange('Gender', id)
            const g = genders.find((x) => x.value === id)
            onMetaChange?.({
              lang,
              gender: { id, displayName: lang === 'en' ? g?.textEn : g?.textBn },
            })
          }}
        >
          <SelectTrigger
            className={`bg-white shadow-[0px_0px_5px_0px_#00000040] rounded-[10px] px-5 py-5 xl:px-6 xl:py-6 ${fieldErrors.Gender ? 'border-red-500 border-2' : ''}`}
          >
            <SelectValue placeholder={L('Select Gender *', 'লিঙ্গ নির্বাচন করুন *')} />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="col-span-2 md:col-span-1">
        <Input
          ...
          value={formData.annualIncome || ''}
          onChange={(e) => handleInputChange('annualIncome', parseInt(e.target.value) || 0)}
          className={`bg-white shadow-[0px_0px_5px_0px_#00000040] rounded-[10px] px-5 py-5 xl:px-6 xl:py-6 ${fieldErrors.annualIncome ? 'border-red-500 border-2' : ''}`}
        />
        {getFieldErrorMessage('annualIncome') && (
          <p className="text-red-500 text-xs mt-1">{getFieldErrorMessage('annualIncome')}</p>
        )}
      ...
      <div className="relative col-span-2 md:col-span-1">
        <Input
          ...
          value={formData.SumAssured || ''}
          onChange={(e) => handleInputChange('SumAssured', parseInt(e.target.value) || 0)}
          className={`bg-white shadow-[0px_0px_5px_0px_#00000040] rounded-[10px] px-5 py-5 xl:px-6 xl:py-6 ${fieldErrors.SumAssured ? 'border-red-500 border-2' : ''}`}
        />
        {getFieldErrorMessage('SumAssured') ? (
          <p className="text-red-500 text-xs mt-1">{getFieldErrorMessage('SumAssured')}</p>
        ) : (
      ...
      >
            ...
            if (method) {
              ...
              onMetaChange?.({
                ...
                payment: {
                  ...
                  displayName: localizePaymode(method.paymode_name, lang),
                },
              })
            }
          }}
        >
          <SelectTrigger
            className={`bg-white shadow-[0px_0px_5px_0px_#00000040] rounded-[10px] px-5 py-5 xl:px-6 xl:py-6 ${fieldErrors.PaymentMode ? 'border-red-500 border-2' : ''}`}
          >
            <SelectValue
              placeholder={
                isLoadingPaymentModes
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {availablePaymentModes.map((pm) => (
                <SelectItem key={pm.paymode_id} value={String(pm.paymode_id)}>
                  {localizePaymode(pm.paymode_name, lang)}
                </SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
      ...
      {/* ✅ calculation error shown on UI (Block-1) */}
      {error && (
        <div className="col-span-2">
          <div className="rounded-md bg-red-50 border border-red-200 px-3 py-2 text-red-700 text-xs">
            {error}
          </div>
        </div>
      )}
      ...
      {/* Submit */}
      <div className="col-span-2 items-center px-4 flex flex-col gap-6 justify-center">
        <Button
          disabled={isLoading || !agreeTerms}
          aria-disabled={isLoading || !agreeTerms}
          className={[
