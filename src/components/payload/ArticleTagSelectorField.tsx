...
import { GLOBAL_ARTICLE_TAGS_SLUG_AND_TAG } from '@/lib/constants'
...
export function ArticleTagSelectorField({ path, field }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchTags = async () => {
      try {
        ...
        const res = await fetch(`/api/globals/${GLOBAL_ARTICLE_TAGS_SLUG_AND_TAG}?depth=0`, {
          credentials: 'include',
          cache: 'no-store',
        })

        if (!res.ok) {
          throw new Error(`Failed to load article tags: ${res.status}`)
        }
        ...
        setTagFetchError((error as Error).message || 'Failed to load article tags.')
      } finally {
    ...
    <div style={{ width: '100%' }}>
      ...
      >
        ...
        >
          ...
          >
            Select Article Tags <span style={{ color: '#D92D20' }}>*</span>
          </div>
        ...
        {loadingTags ? (
          ...
          >
            No article tags found. Please add tags from the Article Tags global.
          </div>
        )}
      ...
      <p style={{ marginTop: 6, fontSize: 12, color: 'var(--theme-elevation-500)' }}>
        Select one or more tags from the Article Tags global list.
      </p>
    </div>
  )
}

export default ArticleTagSelectorField
