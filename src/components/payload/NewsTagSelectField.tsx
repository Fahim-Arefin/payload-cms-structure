...
export function NewsTagSelectField({ path, field }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchTags = async () => {
      try {
        ...
        const rows: TagOption[] = Array.isArray(data?.tags) ? data.tags : []
        ...
        if (!mounted) return
        setFetchError((error as Error).message || 'Failed to load tags.')
      } finally {
  ...
  const selectedTag = useMemo(() => tags.find((item) => item.key === value), [tags, value])
  ...
  return (
    ...
    >
      ...
      {selectedTag ? (
        ...
        >
          <div style={{ fontWeight: 600 }}>{selectedTag.label}</div>
          <div style={{ marginTop: 3 }}>Key: {selectedTag.key}</div>
        </div>
      ) : null}
...
export default NewsTagSelectField
